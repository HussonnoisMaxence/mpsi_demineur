...
def creation_tableau_drapeau(lignes,colonnes):        #Si une case a un drapeau son numéro vaut 1, sinon son numero vaut 0       
    global tableau_drapeau
    tableau_drapeau=np.zeros((lignes+2,colonnes+2),int)
    for k in range(0,lignes+2):         #Toujours 100 pour les bordures
        tableau_devoilement_cases[k,0]=100
        tableau_devoilement_cases[k,colonnes+1]=100
    for k in range(0,colonnes+2):
...
def mise_en_place_plateau_depart(niveau):       #Synthèse de ttes les fns precedentes
    ...
    creation_tableau_drapeau(lignes,colonnes)
...
def gestion_donnees_clic_souris(Donnees_clic):
    ...
    if button==3:
        ...
        afficher_mine(Donnees_clic_arrangees[:2])
...
def afficher_mine(position_mine):
    chdir('C:\\Users\\Jeremy\\Desktop\\Projet_Demineur\\Images')
    mine_fond_blanc=mpimg.imread('Mine_contour_blanc.png')
    abs=position_mine[1]-1
    ord=lignes-position_mine[0]
    if tableau_devoilement_cases[position_mine[0],position_mine[1]]==1:
        plt.imshow(mine_fond_blanc,extent=(abs,abs+1,ord,ord+1))
        tableau_devoilement_cases[position_mine[0],position_mine[1]]=0
    plateau_interactif()