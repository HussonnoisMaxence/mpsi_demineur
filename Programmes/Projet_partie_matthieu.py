...
Dossier_source='C:\\Users\\Matthieu Guinet\\Desktop\\Projet_Demineur(2)\\Projet_Demineur'
...
def jeu():
    ...
    img = plt.imread("menu_choix_niveau.png") #Lecture de l'image
    fig = plt.figure()
    imgplot = plt.imshow(img) #Affichage


    def onclick(event):
        global absisse_clic, ordonne_clic
        absisse_clic = event.xdata
        ordonne_clic = event.ydata
        
        
        if 673<absisse_clic<969 and 496<ordonne_clic<596 and button==1 :  
            mise_en_place_plateau_depart(1)
        if 589<absisse_clic<1049 and 649<ordonne_clic<762 and button==1 :     
            mise_en_place_plateau_depart(2)
        if 663<absisse_clic<979 and 789<ordonne_clic<906 and button==1 :     
            mise_en_place_plateau_depart(3)
    
    interaction_souris = fig.canvas.mpl_connect('button_press_event', onclick)        