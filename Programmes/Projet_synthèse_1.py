...
def creation_coordonnees_des_mines_aleatoirement(lignes,colonnes,nombre_de_mines):
    ...
    while compteur < nombre_de_mines:       #boucle de creation des mines. C'est surement possible de le faire avec un for, mais je suis moins à l'aise avec     #C'est parfait comme ça
        coordonee_de_la_mine = [random.randint(1,abscisse_limite),random.randint(1,ordonnee_limite)]         #random.randint prend un entier aléatoire entre (a,b)
        if  coordonee_de_la_mine in coordonnees_des_mines:       #test pour ne pas avoir deux mines au même endroit
            compteur =compteur
        else:
            coordonnees_des_mines.append(coordonee_de_la_mine)
            compteur =compteur+1
    return coordonnees_des_mines
...
def mise_en_place_plateau_depart(niveau):       #Synthèse de ttes les fns precedentes
    ...
    coordonnees_des_mines=creation_coordonnees_des_mines_aleatoirement(colonnes,lignes,nombre_de_mines)
...
def plateau_interactif(): 
    ...
    if 0<xdata<colonnes and 0<ydata<lignes:     #Si le"clic" est dans la grille...
        L=[xdata,ydata,button]
        figure1.canvas.mpl_disconnect(reaction_souris)      #On stoppe la réaction à la souris
        print("abscisse(±colonne du clic): ", L[0], "   ordonnée(±12-ligne du clic): ", L[1], "   Numero du bouton: ", L[2])        #Et on retourne les coordonnées et le numéro du bouton
    else:
        print('Aïe')        #Sinon on écrit Aïe (c'est temporaire)...
        
        
def afficher_mine(abscisse,ordonnée):
    chdir('C:\\Users\\Jeremy\\Desktop\\Projet_Demineur\\Images')
